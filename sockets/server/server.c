...
void * send_message(void * arg) 
{
  ...
  while (1) 
  {
    ...
    if (result[0] == '0') {  printf("No command\n");} 
    else 
    {
        for (int i = 0; i < countOfClients; ++i) 
        {
           ...
           else 
           {
              ...
              for (int j = 0; j < count; ++j) 
              {
                 ...
                 for (int i = 0; i < 3; ++i) { message[6 + i] = name[i];}
                 message[9] = '_';
                 for (int i = 0; i <= strlen(command); ++i) {message[10 + i] = command[i]; }
                 printf("%s\n", message);
                 if (strncmp(message, "/priv", 5) != 0) broadcastMessage(message);
    	         else 
    	         {
	            char name[4] = "   \0";
	            for (int i = 0; i < 3; ++i) {name[i] = message[i + 6];}
    ...
    delay(5000); countTime++; if(countTime>20){countTime=0; system("clear");}
  }//end while
}//end function
