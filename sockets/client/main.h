...
void CodeNumber(char *str)
{
  ...
  CodeErr=0;
  for (int i=0;i<17;i++) if(ArrOfErr[i]) CodeErr |= (1 << i);
...
void WriteErrMes(char *mes,int flag)
{
  ...
  if(!flag)
  {
    for (int i=0;i<17;i++) {if (ArrOfErr[i]) {str[190+2*i]='1';} else str[190+2*i]='0';}
...
void ReadAvailProd()
{
  ...
  {   k=0; j=0;
      ...
      {
        if(ArrForCopy[i]!=' ') {str[k++]=ArrForCopy[i];}
        else {
               AvailProd[j++]=atoi(str);
               k=0; memset(str,0,1024);
               if(j>4) break;
             }
      }

  } else {sms(ERRTIME("Cannot read database"));exit;}
}
//*****************************************************************************
...
void ReadFlagErr()
{
  ...
  {
    j=0;
    while(ArrForCopy[j]!=' '||j==strlen(ArrForCopy)) j++;
    for (int i=0;i<17;i++) {ArrOfErr[i]=ArrForCopy[++j]-48;j++;}  // Читаем значения флагов ошибок при запуске
  } else {sms(ERRTIME("Cannot read database"));exit;}
}
//******************************************************************************
...
void InOutInit()
{
    ...
    //-------------------------------------------------------------
    DW(RES_TER,LOW);DW(LATCH,LOW);DW(CLOCK,LOW);DW(ELEC_LOCK,LOW);
    RGB(WHITE,ALL);
}
...
void Actuator(int n_act,int pos)
{
    ...
    if((n_act==1)||(n_act==2)||(n_act==3)||(n_act==4))
    {
      switch (n_act)
      {
        case 1: {numact=R_1ACT;if(pos){sens=S_VID_ACT_1;}else{sens=S_ZAD_ACT_1;}break;}
        case 2: {numact=R_2ACT;if(pos){sens=S_VID_ACT_2;}else{sens=S_ZAD_ACT_2;}break;}
        case 3: {numact=R_3ACT;if(pos){sens=S_VID_ACT_3;}else{sens=S_ZAD_ACT_3;}break;}
        case 4: {numact=R_4ACT;if(pos){sens=S_VID_ACT_4;}else{sens=S_ZAD_ACT_4;}break;}
      }

      if (pos)      {DW(REVERS,LOW);DW(numact,HIGH);}  //Если выдвинуть
      if (pos==LOW) {DW(REVERS,HIGH);DW(numact,LOW);}  //Если задвинуть
      if(ArrOfErr[F_POWER]==0) CountTime=25;  else CountTime=50;  // Питание от батареи или от БП
      while(!DSR[sens]&&CountTime) {delay(200);CountTime--;}
...
void LowRgb(int num_button)
{
  switch(num_button)
  {
    case ALL:
    {
      DW(R_1,LOW);DW(G_1,LOW);DW(B_1,LOW);DW(R_2,LOW);DW(G_2,LOW);DW(B_2,LOW);
      DW(R_3,LOW);DW(G_3,LOW);DW(B_3,LOW);DW(R_4,LOW);DW(G_4,LOW);DW(B_4,LOW);break;
    }
    case 1:{DW(R_1,LOW);DW(G_1,LOW);DW(B_1,LOW);break;}
    case 2:{DW(R_2,LOW);DW(G_2,LOW);DW(B_2,LOW);break;}
    case 3:{DW(R_3,LOW);DW(G_3,LOW);DW(B_3,LOW);break;}
    case 4:{DW(R_4,LOW);DW(G_4,LOW);DW(B_4,LOW);break;}
  }
}
/************************************************************************/
// Задержка в секундах
...
void del(int idel)
{
  for (int i=0;i<idel;i++) { delay(1000);}
}
/************************************************************************/
...
void RGB(int color,int num_button)
{
  if (color==WHITE)
  {
    switch(num_button)
    {
      case ALL:
      {
       DW(R_1,HIGH);DW(G_1,HIGH);DW(B_1,HIGH);DW(R_2,HIGH);DW(G_2,HIGH);DW(B_2,HIGH);
       DW(R_3,HIGH);DW(G_3,HIGH);DW(B_3,HIGH);DW(R_4,HIGH);DW(G_4,HIGH);DW(B_4,HIGH);break;
      }
      case 1:{DW(R_1,HIGH);DW(G_1,HIGH);DW(B_1,HIGH);break;}
      case 2:{DW(R_2,HIGH);DW(G_2,HIGH);DW(B_2,HIGH);break;}
      case 3:{DW(R_3,HIGH);DW(G_3,HIGH);DW(B_3,HIGH);break;}
      case 4:{DW(R_4,HIGH);DW(G_4,HIGH);DW(B_4,HIGH);break;}
    }
  }
//-------------------------------------------------------------
  if (color==RED)
  {
    switch(num_button)
    {
      case ALL:
      {
       DW(R_1,HIGH);DW(G_1,LOW);DW(B_1,LOW);DW(R_2,HIGH);DW(G_2,LOW);DW(B_2,LOW);
       DW(R_3,HIGH);DW(G_3,LOW);DW(B_3,LOW);DW(R_4,HIGH);DW(G_4,LOW);DW(B_4,LOW);break;
      }
      case 1:{DW(R_1,HIGH);DW(G_1,LOW);DW(B_1,LOW);break;}
      case 2:{DW(R_2,HIGH);DW(G_2,LOW);DW(B_2,LOW);break;}
      case 3:{DW(R_3,HIGH);DW(G_3,LOW);DW(B_3,LOW);break;}
      case 4:{DW(R_4,HIGH);DW(G_4,LOW);DW(B_4,LOW);break;}
    }
  }
//-------------------------------------------------------------
  if (color==GREEN)
  {
    switch(num_button)
    {
      case ALL:
      {
       DW(R_1,LOW);DW(G_1,HIGH);DW(B_1,LOW);DW(R_2,LOW);DW(G_2,HIGH);DW(B_2,LOW);
       DW(R_3,LOW);DW(G_3,HIGH);DW(B_3,LOW);DW(R_4,LOW);DW(G_4,HIGH);DW(B_4,LOW);break;
      }
      case 1:{DW(R_1,LOW);DW(G_1,HIGH);DW(B_1,LOW);break;}
      case 2:{DW(R_2,LOW);DW(G_2,HIGH);DW(B_2,LOW);break;}
      case 3:{DW(R_3,LOW);DW(G_3,HIGH);DW(B_3,LOW);break;}
      case 4:{DW(R_4,LOW);DW(G_4,HIGH);DW(B_4,LOW);break;}
    }
  }
//-------------------------------------------------------------
  if (color==BLUE)
  {
    switch(num_button)
    {
      case ALL:
      {
       DW(R_1,LOW);DW(G_1,LOW);DW(B_1,HIGH);DW(R_2,LOW);DW(G_2,LOW);DW(B_2,HIGH);
       DW(R_3,LOW);DW(G_3,LOW);DW(B_3,HIGH);DW(R_4,LOW);DW(G_4,LOW);DW(B_4,HIGH);break;
      }
      case 1:{DW(R_1,LOW);DW(G_1,LOW);DW(B_1,HIGH);break;}
      case 2:{DW(R_2,LOW);DW(G_2,LOW);DW(B_2,HIGH);break;}
      case 3:{DW(R_3,LOW);DW(G_3,LOW);DW(B_3,HIGH);break;}
      case 4:{DW(R_4,LOW);DW(G_4,LOW);DW(B_4,HIGH);break;}
    }
  }
//-------------------------------------------------------------
}//end RGB
  ...
  //  printf("%s%s\n","AT+CMGS=","+79529174466");
   ...
   if ((fd = serialOpen ("/dev/MyDevices/Modem", 115200)) < 0)       //
    ...
    if(!ReadMysqlTable(GiveMeTelNumber,numbers))  //если открыта бд с номерами телефонов
    {
        ...
        {
            memset(Buf,0,13);
            memmove (&Buf[0], &numbers[j], 12); j=j+15; Buf[12]=0;
            printf("%d %d ",strlen(numbers),j);
            serialPuts(fd,"AT+CMGD=1,4\r"); printf("%s\n","AT+CMGD=1,4\r");
            while(!r_uart(fd)){delay(100);i++;if (i>50) {printf("Modem not answer\n");i=0;break;}};i=0;
            serialPuts(fd,"AT+CSCS=\"GSM\"\r"); printf("%s\n","AT+CSCS=\"GSM\"\r");
            while(!r_uart(fd)){delay(100);i++;if (i>50) {printf("Modem not answer\n");i=0;break;}};i=0;
            serialPuts(fd,"AT+CMGF=1\r"); printf("%s\n","AT+CMGF=1\r");
            while(!r_uart(fd)){delay(100);i++;if (i>50) {printf("Modem not answer\n");i=0;break;}};i=0;
            serialPuts(fd,"AT+CMGS=");
            serialPuts(fd,"\"");
            serialPuts(fd,Buf);
            serialPuts(fd,"\"");
            serialPuts(fd,"\r"); printf("%s%s\n","AT+CMGS=",Buf);
            while(!r_uart(fd)){delay(100);i++;if (i>50) {printf("Modem not answer\n");i=0;break;}};i=0;
            serialPuts(fd,message); printf("%s\n",message);
            delay(1500);
            serialPutchar(fd,0x1A); printf("%s\n","0x1A");
            while(!r_uart(fd)){delay(100);i++;if (i>50) {printf("Modem not answer\n");i=0;break;}};i=0;
        }
    }
...
int TryConnection(int flag)
{
  int i,fd;
      ...
      else
      {
         ...
         {
           i=0;
           gen_IDL(&req);sendUART(&req,fd);delay (100);
           while (!read_from_UART(&req,fd))
           {
              delay(100);i++;
              if(i>70){i=0;count_1++;printf("Terminal not answer \n");break;}
           }
           ...
           if(i>0)
           {
             ...
             if(ArrOfErr[F_TERMIN]) {count_1=0;count_2=0; ArrOfErr[F_TERMIN]=0;
              flag_message=1; WriteErrMes("Terminal ok",0);}
         ...
         if (flag==2)
         {
            ...
            {
               delay(100);i++;
               if(i>50){i=0;count_2++;printf("Terminal not answer \n");break;}
            }
         ...
         {
            i=0;
            gen_IDL_SCREEN_EMPTY(&req); sendUART(&req,fd);delay (100);
            while (!read_from_UART(&req,fd))
            {
               delay(100);i++;
               if(i>30){i=0;printf("Terminal not answer \n");break;}
            }
            return fd;
         }//end if (flag==3)
         ...
         {
            i=0;
            gen_IDL_SCREEN_OWNER(&req); sendUART(&req,fd);
            while (!read_from_UART(&req,fd))
            {
               delay(100);i++;
               if(i>30){i=0;printf("Terminal not answer \n");break;}
...
char *translit(char *instr)
{
  ...
  {
    if (instr[i]==0xd0&&instr[i+1]==0x90)  {outstr[j++]='A'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x91)  {outstr[j++]='B'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x92)  {outstr[j++]='V'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x93)  {outstr[j++]='G'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x94)  {outstr[j++]='D'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x95)  {outstr[j++]='E'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x96)  {outstr[j++]='J'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x97)  {outstr[j++]='Z'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x98)  {outstr[j++]='I'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x99)  {outstr[j++]='Y'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x9A)  {outstr[j++]='K'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x9B)  {outstr[j++]='L'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x9C)  {outstr[j++]='M'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x9D)  {outstr[j++]='N'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x9E)  {outstr[j++]='O'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0x9F)  {outstr[j++]='P'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA0)  {outstr[j++]='R'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA1)  {outstr[j++]='S'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA2)  {outstr[j++]='T'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA3)  {outstr[j++]='U'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA4)  {outstr[j++]='F'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA5)  {outstr[j++]='X'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA6)  {outstr[j++]='C'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA7)  {outstr[j++]='C'; outstr[j++]='H'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA8)  {outstr[j++]='S'; outstr[j++]='H'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xA9)  {outstr[j++]='S'; outstr[j++]='H'; outstr[j++]='`'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xAA)  {outstr[j++]='`'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xAB)  {outstr[j++]='Y';outstr[j++]='`';i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xAC)  {outstr[j++]='`'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xAD)  {outstr[j++]='E';outstr[j++]='`'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xAE)  {outstr[j++]='Y';outstr[j++]='U'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xAF)  {outstr[j++]='Y';outstr[j++]='A'; i+=2;continue; }
    if (instr[i]==0xd0&&instr[i+1]==0xB0)  {outstr[j++]='a'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB1)  {outstr[j++]='b'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB2)  {outstr[j++]='v'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB3)  {outstr[j++]='g'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB4)  {outstr[j++]='d'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB5)  {outstr[j++]='e'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB6)  {outstr[j++]='j'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB7)  {outstr[j++]='z'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB8)  {outstr[j++]='i'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xB9)  {outstr[j++]='y'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xBA)  {outstr[j++]='k'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xBB)  {outstr[j++]='l'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xBC)  {outstr[j++]='m'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xBD)  {outstr[j++]='n'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xBE)  {outstr[j++]='o'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0xBF)  {outstr[j++]='p'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x80)  {outstr[j++]='r'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x81)  {outstr[j++]='s'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x82)  {outstr[j++]='t'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x83)  {outstr[j++]='u'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x84)  {outstr[j++]='f'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x85)  {outstr[j++]='x'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x86)  {outstr[j++]='c'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x87)  {outstr[j++]='c'; outstr[j++]='h'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x88)  {outstr[j++]='s'; outstr[j++]='h'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x89)  {outstr[j++]='s'; outstr[j++]='h';outstr[j++]='`';i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x8A)  {outstr[j++]='`'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x8B)  {outstr[j++]='y'; outstr[j++]='`'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x8C)  {outstr[j++]='`'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x8D)  {outstr[j++]='e'; outstr[j++]='`'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x8E)  {outstr[j++]='y'; outstr[j++]='u'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x8F)  {outstr[j++]='y'; outstr[j++]='a'; i+=2;continue;}
    if (instr[i]==0xd0&&instr[i+1]==0x81)  {outstr[j++]='E'; outstr[j++]='`'; i+=2;continue;}
    if (instr[i]==0xd1&&instr[i+1]==0x91)  {outstr[j++]='e'; outstr[j++]='`'; i+=2;continue;}
    outstr[j++]= instr[i++];
  } //end while
