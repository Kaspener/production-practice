...
void *receive_messages(void *arg)
{
  ...
  while (1)
  {
    ...
    else
    {
      ...
      {
        delay(100);
        sendall(socket1, "ok", strlen("ok"), 0);
      }
      ...
      if (strncmp(buffer, "BURN", 4) == 0)
      {
        ...
        del(2);
        {
          ...
          {
            WriteErrMes("sowtware firmvare is new, now vend device will be reboot", 1);
            del(1);
            sync();
            reboot(RB_AUTOBOOT);
          }
...
void *send_message(void *arg)
{
  ...
  while (1)
  {
    ...
    case 0:
    {
      ...
      {
        countTime = 25000;
        sendall(socket1, "en", strlen("en"), 0);
      }
      break;
    } // "Поддержка" связи переодической отправкой сообщений на сервер
    case 1:
    {
    ...
    if (!F_CLIENT_SEND)
    {
      F_CLIENT_SEND = 1;
      break;
    } // если обрыв связи с сервером выходим из поточной функции
  }
}
/************************************************************************/
//
...
void *Client(void *args)
{
  ...
  while (1)
  {
    ...
    {
      printf("(Re) Conecting...\n");
      memset(NAMECLIENT, 0, sizeof(NAMECLIENT));
      i = 0;
      j = 2;
      ReadMysqlTable(inf_app, ArrForCopy); // Читаем инфу аппарата из БД
      while (ArrForCopy[j] != ' ')
      {
        NAMECLIENT[i++] = ArrForCopy[j++];
        if (i > 4)
          break;
      }
      if (strlen(NAMECLIENT) == 1)
      {
        NAMECLIENT[2] = NAMECLIENT[0];
        NAMECLIENT[1] = '0';
        NAMECLIENT[0] = '0';
      }
      if (strlen(NAMECLIENT) == 2)
      {
        NAMECLIENT[2] = NAMECLIENT[1];
        NAMECLIENT[1] = NAMECLIENT[0];
        NAMECLIENT[0] = '0';
      }
      ...
      delay(700);
      flag_message = 1;
      delay(700); // При подключении к серверу отправляем все текущие ошибки аппарата
      flag_message = 2;
      delay(700); // При подключении к серверу отправляем количество продуктов
      if (ArrOfErr[F_SERVER])
      {
        ArrOfErr[F_SERVER] = 0;
...
void *Terminal(void *args)
{
  ...
  {
    F_SELLING = 1;
    if (!ArrOfErr[2 * i + 3] && !ArrOfErr[2 * i + 4])
    {
      Actuator(i, HIGH);
    }
    F_SELLING = 0;
  ...
  while (1)
  {
    ...
    AGAIN:
      ...
      case 1:
      {
        gen_VRP(&req, PRICE1, IDITEM1, NAME1);
        break;
      }
      case 2:
      {
        gen_VRP(&req, PRICE2, IDITEM2, NAME2);
        break;
      }
      case 3:
      {
        gen_VRP(&req, PRICE3, IDITEM3, NAME3);
        break;
      }
      case 4:
      {
        gen_VRP(&req, PRICE4, IDITEM4, NAME4);
        break;
      }
      }
      sendUART(&req, fd);
      ...
      del(1); // запрещаем выбор товара

      if (chekSEAL(&req)) // если оплата прошла успешно
      {
        dfplayer_SetTrakNumber(4);
        del(2);                    // товар оплачен
        get_brecpt(receipt, &req); // получаем чек
        dfplayer_SetTrakNumber(5);
        del(2); // электронный чек на экране терминала
        gen_IDL_QR(&req, receipt);
        sendUART(&req, fd); // qr cod на экран терминала

        // Выдаем товар задвигая и затем выдвигая соответствующий актуатор
        Actuator(NUM_BUT, LOW);
        gen_IDL_QR(&req, receipt);
        sendUART(&req, fd);
        Actuator(NUM_BUT, HIGH);
        gen_IDL_QR(&req, receipt);
        sendUART(&req, fd);
        ...
        if (!DSR[S_TRAY]) // Если не сработал датчик в лотке выдачи
        {
          ...
          case 1:
          {
            gen_FIN(&req, 0, IDITEM1, NAME1);
            break;
          }
          case 2:
          {
            gen_FIN(&req, 0, IDITEM2, NAME2);
            break;
          }
          case 3:
          {
            gen_FIN(&req, 0, IDITEM3, NAME3);
            break;
          }
          case 4:
          {
            gen_FIN(&req, 0, IDITEM4, NAME4);
            break;
          }
          }
          sendUART(&req, fd);
        ...
        else // иначе финализируем покупку
        {
          ...
          while (DSR[S_TRAY])        // Пока не забрали товар ждем
          {
            ...
            if (i == 180)
            {
              WriteErrMes("Buyer did not pick up product", 1);
            } // Если товар не забрали в течении 3 минут
          ...
          i = 0;
          del(5); // время ожидания до закрытия замка на лотке выдачи  5 sek
          NUM_BUT = 0;
          F_SELLING = 0;
          count_time = 40001;
          COUNT_OF_ERROR_ACT = 0;
      ...
      else
      {
        if (get_brecpt(receipt, &req)) // если нет средств
        {
          ...
          dfplayer_SetTrakNumber(3);
          del(2); // Товар не оплачен попробуйте еще раз
          if (fd >= 0)
            serialClose(fd);
          delay(300);
          goto AGAIN;
...
int main()
{
  ...
  while (1)
  {
    ...
    button[1] = DSR[BUT_1];
    sensor[1] = DSR[S_PROD_1];
    button[2] = DSR[BUT_2];
    sensor[2] = DSR[S_PROD_2];
    button[3] = DSR[BUT_3];
    sensor[3] = DSR[S_PROD_3];
    button[4] = DSR[BUT_4];
    sensor[4] = DSR[S_PROD_4];
    ...
    if (F_SENS_AVAIL_PROD)
    {
      ...
      for (k = 1; k < 5; k++)
      {
        ...
        {
          ArrOfErr[k + 12] = 1;
          snprintf(str, 30, "Sensor product %d %s", k, "error");
          flag_message = 1;
          WriteErrMes(str, 0);
        }
        ...
        {
          ArrOfErr[k + 12] = 0;
          snprintf(str, 30, "Sensor product %d %s", k, "ok");
          flag_message = 1;
          WriteErrMes(str, 0);
        }
      }
    ...
    for (k = 0; k < 4; k++)
    {
      if (ArrOfErr[0] || ArrOfErr[1] || ArrOfErr[2] || ArrOfErr[4])
      {
        RGB(RED, ALL);
        break;
      }
      if (!sensor[k + 1] || !AvailProd[k + 1] || ArrOfErr[k * 2 + 5] || ArrOfErr[k * 2 + 6] || ArrOfErr[k + 13])
      {
        RGB(RED, k + 1);
      }
      else
      {
        k == (NUM_BUT - 1) ? RGB(GREEN, k + 1) : RGB(WHITE, k + 1);
      }
    }
      ...
      while ((k >= 1) && (!F_SELLING) && (!DSR[S_TRAY]) && !ArrOfErr[F_TRAY] && !ArrOfErr[F_SERIAL] && !ArrOfErr[F_TERMIN])
      {
        if (button[k])
        {
          for (i = 0; i < 4; i++)
          {
            ...
            RGB(WHITE, k);
            delay(200);
          } // мигаем белым цветом выбранной кнопки
